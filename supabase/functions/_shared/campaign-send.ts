...
// FIX (2026-09): the "activation" campaign (Monthly Activity Campaign / this
// tick) and the manual Monthly Activity Push page + send-activity-appreciation
// function were running as two fully independent systems with no shared
// dedupe. This function only wrote to activation_campaign_recipients +
// campaign_send_ledger; the Push page's "Done" detection and
// send-activity-appreciation's entire dedupe check both read ONLY
// contact_activities. Result: an entry the automated tick already sent still
// showed as "Pending" everywhere else, so the same person could be sent a
// second, differently-worded appreciation message via the manual path.
//
// This mirrors every real "activation" send into contact_activities using the
// exact marker format send-activity-appreciation and MonthlyActivityPush.tsx
// already understand: [monthly_activity_appreciation:<YYYY-MM>]
// [monthly_activity_appreciation_entry:oid:<order_id>] [maytapi_message:<id>].
// Best-effort — must never block or fail the actual send.
async function mirrorActivationToContactActivities(params: {
  userId: string;
  contactId: string | null;
  orderId: string | null;
  monthKey: string;
  ...
  try {
    if (!params.contactId || !params.orderId) return; // nothing to key the shared marker on
    const monthMarker = `[monthly_activity_appreciation:${params.monthKey}]`;
    const entryMarker = `[monthly_activity_appreciation_entry:oid:${params.orderId}]`;
    const msgMarker = `[maytapi_message:${params.msgId ?? "unknown"}]`;
    await admin.from("contact_activities").insert({
      user_id: params.userId,
      contact_id: params.contactId,
      activity_type: "whatsapp",
      summary:
        `Sent monthly activity appreciation [activation-campaign-tick] | ${params.monthKey} | ` +
        `${monthMarker} ${entryMarker} ${msgMarker}`,
      notes:
        `Sent automatically by the Monthly Activity Campaign (tiered, activation-campaign-tick).` +
        (params.amount != null ? ` Amount: R${params.amount}.` : "") +
        (params.monthLabel ? ` Cycle: ${params.monthLabel}.` : "") +
        `\n\n${monthMarker} ${entryMarker} ${msgMarker}`,
      next_action: "",
    });
  } catch (_e) {
    // swallow — must never block or fail the real send path
  }
}
...
export async function runCampaignTick(opts: TickOptions) {
  ...
  for (const row of rows ?? []) {
    ...
    // FIX (2026-09): keep the "activation" campaign's sends visible to every
    // other system that reads contact_activities as truth (Monthly Activity
    // Push page + send-activity-appreciation's dedupe). See comment on
    // mirrorActivationToContactActivities above for the full story.
    if (opts.campaignKey === "activation") {
      await mirrorActivationToContactActivities({
        userId: (row as any).user_id,
        contactId: (row as any).contact_id ?? null,
        orderId: (row as any).order_id ?? null,
        monthKey: cycleKey,
        msgId: send.msgId,
        amount: (row as any).amount != null ? Number((row as any).amount) : undefined,
        monthLabel: (row as any).activity_month ?? undefined,
      });
    }
